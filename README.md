...
Pods currently don't have the flexibility to do what's best for the pod.  Webstore-2 relies on zapi that was designed for Webstore-1. We're constantly fighting with the odd ways of how zapi and webstore-1 did thing in the past.  For example, when a user enters an address we need to tell zapi about this so we can get an address_id back. This address_id is useless to webstore but needed upon checkout. Too much logic is being performed on the frontend to determine the state of a registering user; zapi has all this information.  We could go into zapi and update these routes but run the real risk of `.finally is not a function` that our false positive test failed to find because we didn't have chai configured correctly and the moon phase was in its first quarter phase over Easter Island.
...
Another approach is to group your code by it's functional type. For example, all your handlers go in one package, your controllers go in another, and your models go in yet another.
...
Let's say we want to test a http handler
```go
